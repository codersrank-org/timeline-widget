...
const ESM_EXPORT = `
export default CodersrankTimeline;
`;
...
const buildModule = async (cssContent, format) => {
  ...
  const fileContent = fs
    .readFileSync(`./${outputDir}/${format}/codersrank-timeline.js`, 'utf8')
    .replace('$_STYLES_$', cssContent)
    .replace('// EXPORT', format === 'esm' ? ESM_EXPORT : CJS_EXPORT);
  fs.writeFileSync(`./${outputDir}/${format}/codersrank-timeline.js`, fileContent);
};

const buildUMD = async (cssContent) => {
  const env = process.env.NODE_ENV || 'development';
  const filename = 'codersrank-timeline';
  const outputDir = env === 'development' ? 'dev' : 'package';
  ...
  const bundle = await rollup({
    input: './src/codersrank-timeline.js',
    plugins: [
      replace({
        delimiters: ['', ''],
        $_STYLES_$: cssContent,
...
const build = async () => {
  let cssContent = fs.readFileSync('./src/codersrank-timeline.scss', 'utf-8');
